...
def get_song_info(xmlUrl):
    ...
    #下面用来获取歌曲信息
    title = root.xpath('/xm:playlist/xm:trackList/xm:track/xm:songName',namespaces={"xm": 'http://xspf.org/ns/0/'})[0].text
    albumName =  root.xpath('/xm:playlist/xm:trackList/xm:track/xm:albumId',namespaces={"xm": 'http://xspf.org/ns/0/'})[0].text
    ...
    location = root.xpath('/xm:playlist/xm:trackList/xm:track/xm:location',namespaces={"xm": 'http://xspf.org/ns/0/'})[0].text
    #print(title,albumName,artist,location)
    return title, int(albumName), int(artist), location
...
def get_song_full_info(songId):
    xmlUrl = find_xml(songId)
    #获取普通音质歌曲的下载地址

    location = get_song_info(xmlUrl)
    #对下载地址进行解密
    downloadUrl = decode_location(location[3])
    return (location[0],location[1],location[2], downloadUrl)
...
def download_hq_music(songUrl):
    ...
    #设置歌曲保存地址为当前工作空间
    #address = os.getcwd()+'/'+title+'h.mp3'
    #urllib.urlretrieve(hqDownloadUrl, address )
    address = os.getcwd()+'/'+title+'.mp3'
    urllib.urlretrieve(downloadUrl, address )
    #print 'high quality ' + '"' + title +  '"' + ' has been downloaded'

#http://www.xiami.com/album/bqhKBJ03c141?spm=a1z1s.6843761.1110925389.7.oU8RhK
